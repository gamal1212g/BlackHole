...
try:
    ...
    logger.error(
        "Scapy is not installed. Please install it using 'pip install scapy'.")
    sys.exit(1)
...
class IDSDetector:
    ...
    def analyze_packet(self, packet):
        ...
        with self.lock:
            ...
            # land attack check
            if packet.haslayer(TCP):
                if src_ip == dst_ip and packet[TCP].sport == packet[TCP].dport:
                    attack_label = "Land Attack"
                    alert_triggered = True

            # xmas scan check
            if not alert_triggered and packet.haslayer(TCP):
                if packet[TCP].flags == 0x29:  # FPU flags
                    self.xmas_history[src_ip].append(now)
                    if len(self.xmas_history[src_ip]) > self.xmas_threshold:
                        attack_label = "Xmas Scan"
                        alert_triggered = True
            ...
            if not alert_triggered and packet.haslayer(TCP):
                if packet[TCP].flags == "F":
                    ...
                    if len(self.fin_history[src_ip]) > self.fin_threshold:
                        attack_label = "FIN Scan"
                        alert_triggered = True

            # smurf attack check
            if not alert_triggered and packet.haslayer(ICMP):
                if packet[ICMP].type == 8:  # echo req
            ...
            # udp flood check
            if not alert_triggered and packet.haslayer(UDP):
            ...
            if not alert_triggered and (packet.haslayer(TCP) or packet.haslayer(UDP)):
                if not packet.haslayer(TCP) or packet[TCP].flags == "S":
                    ...
                    # Cleanup and check
                    expired = [
                        p for p, t in self.port_scan_history[src_ip].items() if t < now - self.window]
                    for p in expired:
            ...
            if alert_triggered and attack_label:
                ...
                for history in [self.xmas_history, self.null_history, self.fin_history,
                                self.smurf_history, self.icmp_history, self.udp_flood_history,
                                self.syn_flood_history, self.ddos_history]:
                ...
                if now - self.alert_cooldowns[src_ip][attack_label] > self.cooldown_period:
                    ...
                    # Reset the specific history that triggered the alert
    ...
    def generate_alert(self, src_ip, attack_type):
        ...
        try:
            ...
            requests.post("http://localhost:8000/api/v1/alerts/blocked",
                          json=block_payload, timeout=5)
    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to fetch config. Status: {response.status_code}")
        except Exception as e:
...
def main():
    if not check_privileges():
        logger.error(
            "This script requires root/administrative privileges to sniff network traffic.")
        sys.exit(1)
    ...
    worker = threading.Thread(target=worker_thread, args=(
        packet_queue, detector), daemon=True)
    worker.start()

    # Start config poller thread
    poller = threading.Thread(
        target=config_poller_thread, args=(detector,), daemon=True)
    poller.start()

    logger.info("Sniffing started... Press Ctrl+C to stop.")

    try:
        # scapy sniff needs to target all interfaces on windows
        from scapy.all import conf, get_if_list

        sniff_thread = threading.Thread(
            target=lambda: sniff(
                iface=None,
                prn=lambda p: packet_callback(p, packet_queue),
                store=0
            ),
            daemon=True
        )
        ...
        logger.error(
            f"Permission error during sniffing: {e}. Ensure you have the right drivers (e.g., Npcap) and privileges.")
    except KeyboardInterrupt:
        logger.info("Shutting down...")
    except Exception as e:
        logger.error(f"Unexpected error: {e}")
    finally:
        packet_queue.put(None)  # Stop worker
