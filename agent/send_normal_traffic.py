...
def get_local_ip():
    # نفس الخدعة الذكية لمعرفة الـ IP الحقيقي للجهاز
...
def send_normal_packets():
    ...
    # هنبعت 5 باكتات عادية جداً (Ping / ICMP) وبينهما وقت متباعد عشان السيستم ميفتكرهاش هجوم
    for i in range(1, 6):
        packet = IP(dst=target_ip)/ICMP()
        send(packet, verbose=False)
        print(f"  [+] Normal Packet #{i} sent successfully.")
        time.sleep(2) # وقت راحة طويل (2 ثانية) للتأكيد إنها مش هجوم سريع
