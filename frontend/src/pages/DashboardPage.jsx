...
export default function DashboardPage() {
  ...
  // Enterprise Color Palette for 10 Attacks
  const PIE_COLORS = [
    '#ef4444', // DDoS (Red)
    '#f97316', // Port Scan (Orange)
    '#eab308', // SYN Flood (Yellow)
    '#22c55e', // ICMP Flood (Green)
    '#06b6d4', // UDP Flood (Cyan)
    '#3b82f6', // Xmas Scan (Blue)
    '#6366f1', // FIN Scan (Indigo)
    '#a855f7', // Null Scan (Purple)
    '#d946ef', // Smurf Attack (Pink)
    '#f43f5e', // Land Attack (Rose)
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-300">
      ...
      <div className="flex justify-between items-center bg-[#0d1526] border border-gray-800 p-4 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.5)] border-l-4 border-l-[#00ff9d]">
        ...
        <button 
          onClick={clearAlerts}
          className="px-6 py-2 bg-red-500/10 hover:bg-red-500 hover:text-white text-red-500 text-xs font-black tracking-widest rounded border border-red-500/30 transition-all uppercase"
        >
        ...
        <div className="bg-[#0d1526] border border-gray-800 p-6 rounded-xl flex items-center justify-between shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
          <div>
            ...
            <h3 className="text-3xl font-black text-white">
              {(stats?.scannedTraffic || 0).toLocaleString()}
            </h3>
          </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trafficData}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#070d1a', borderColor: '#1f2937', color: '#fff', borderRadius: '8px' }} 
                ...
                {alerts.length === 0 ? (
                  ...
                  alerts.slice(0, 5).map((alert, index) => (
                    <tr key={index} className="hover:bg-gray-800/20 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-orange-400 font-mono font-bold">
                        {alert.source_ip}
                      </td>
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 rounded bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-bold">
                          {alert.attack_type}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <button 
                          ...
                          className="px-4 py-1.5 bg-gray-800 hover:bg-red-500 hover:text-white text-gray-300 text-xs font-bold rounded transition-colors border border-gray-700 hover:border-red-500 opacity-0 group-hover:opacity-100"
                        >
                          Block IP
                        </button>
                      </td>
                ...
                {blockedIps.length === 0 ? (
                  ...
                  blockedIps.slice(0, 5).map((block, index) => (
                    <tr key={index} className="hover:bg-yellow-500/5 transition-colors">
                      <td className="px-6 py-4 text-yellow-500 font-mono font-bold">
                        {block.ip}
                      </td>
